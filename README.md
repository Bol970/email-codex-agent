...
`npm run demo:record` opens a headed browser, runs the mock presentation
by itself at the default fast pace, tours every Codex action preset, and closes the browser/server after the final title. On Linux/X11 it
...
demo:record` for a slower full-length version or `DEMO_PACE=0.25 npm run
demo:record` for an extra quick take.
